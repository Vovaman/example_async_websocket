...
имеет простую функциональность. Сервер поддерживает только одну команду для клиентов: `ws://<server>/<client_id>`, где
...
Установите Python 3.9 на свой компьютер, если ещё не сделали этого.  
...
2. Определите IP адрес вашего компьютера в wifi-сети, для чего в терминале выполните команду:  
   ...
   $ ip a
   ```
   Команда выведет список всех сетевый драйверов с их параметрами.  
...
```
## Параметры uvicorn для сигналов ping/pong
Ping/pong - это обмен специальными сигналами между сервером и клиентом для поддержания в рабочем состоянии канала связи.  
Параметры для ping/pong определяются при запуске `uvicorn` в командной строке: `--ws-ping-interval 10 --ws-ping-timeout 10`.  
...
ping/pong от сервера клиенту, что приведет к повышению нагрузки на контроллер.  
