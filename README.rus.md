...
```
{
   ...
   "socket_delay_ms": 5                     # задержка для операций чтения/записи данных тестового сервера.
    ...
    "ssl": {
        "ca": "<ca cert in DER-format file name>", # сертификат удостоверяющего центра в DER формате
        "key": "<client key>", # файл с секретным ключом клиента
        "cert": "<client cet>", # файл сертификата клиента
        "cert_reqs": 2 # код режима проверки сертификатов: 0 - CERT_NONE, 1 - CERT_OPTIONAL, 2 - CERT_REQUIRED
    },
}
```
