...
# Запуск проекта
> [!NOTE]
> Перед выполнением всех примеров выясним:
> - Имя (``<wifi network name>``) и пароль ``<wifi network pwd>`` wifi-сети,
> посредством которой связаны компьютер, на котором мы работаем и контроллер;
> - IP-адрес компьютера (``<host IP>``), на котором мы работаем, в указанной wifi-сети.

## WS. Безопасность отсутствует.
1. Исправляем файл ``src/config.json``:
   ```json
   {
      "wifi": {
   ...
   ```
2. Синхронизируем с помощью ``PyMakr`` проект с контроллером.
3. Запускаем сервер:
   ```bash
